...
TEST(BasicTests, InputBasic) {
    ...
    for (int i=0; i < table.size(); i++) {
        EXPECT_EQ(abs(table[i]-match1[i%9]) < .001, true);
    }
    ...
    for (int i=0; i < table.size(); i++) {
        if (i % 2 == 0) {
            EXPECT_EQ(abs(table[i]-match2a[i%4]) < .001, true);
        } else {
            EXPECT_EQ(abs(table[i]-match2b[i%4]) < .001, true);
        }        
    }
